...
function CommoditiesContent() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="relative overflow-hidden border-b border-zinc-200/80 dark:border-zinc-800/80 bg-gradient-to-br from-cyan-50 via-teal-50/50 to-background dark:from-cyan-950/30 dark:via-teal-950/20 dark:to-background">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-20 -right-20 h-60 w-60 rounded-full bg-cyan-200/30 dark:bg-cyan-800/10 blur-3xl" />
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-foreground">{t("category_pages.commodities_title")}</h1>
          </div>
          <p className="text-muted-foreground text-lg max-w-xl">
            {t("category_pages.commodities_subtitle")}
          </p>
        </div>
      </div>
        ...
        <Tabs defaultValue={txn || "all"} className="mb-6">
          <TabsList className="bg-zinc-100 dark:bg-zinc-800/80 rounded-xl p-1">
            <TabsTrigger value="all" asChild className="rounded-lg data-[state=active]:shadow-md">
              <Link href="/commodities">{t("category_pages.tab_all")}</Link>
            </TabsTrigger>
            <TabsTrigger value="buy" asChild className="rounded-lg data-[state=active]:shadow-md">
              <Link href="/commodities?txn=buy">{t("category_pages.tab_buy")}</Link>
            </TabsTrigger>
            <TabsTrigger value="sell" asChild className="rounded-lg data-[state=active]:shadow-md">
              <Link href="/commodities?txn=sell">{t("category_pages.tab_sell")}</Link>
            </TabsTrigger>
          </TabsList>
        </Tabs>

        <div className="flex gap-6">
          <Filters category="commodity" />
          {loading ? (
